...
def getVerticalBoxesBelow(thisindex,rtreeidx,imagetexts,imgheight):
	...
	#print location,extendedloc
	nns=list(rtreeidx.intersection(extendedloc))
	nns=[x for x in nns if x!=thisindex]
	#if imagetext['Text']=='(mm)':
	#	print location,extendedloc,nns
	return len(nns)
...
def getClass(thisindex,rtreeidx,imagetexts,imgwidth,imgheight):
	#print "here"
	if isYaxisValue(thisindex,rtreeidx,imagetexts,imgwidth,imgheight):
		#print thisindex, "returned","yaxisvalue"
		return "yaxisvalue"
	if isXaxisValue(thisindex,rtreeidx,imagetexts,imgwidth,imgheight):
