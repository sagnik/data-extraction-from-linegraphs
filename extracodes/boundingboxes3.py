...
def main():
	...
	#pilImg.show()
	pilImg.save(os.path.join(imgDir,"textextraction/cc",exactImageLoc[:-4]+"-cc.png"))
	'''	
	mybnumbers=0
	for item in finalBoundingboxesFiltered:
	...
	for item in finalBoundingboxes:
		draw.rectangle([item[1],item[0],item[3],item[2]],outline="red")
	#pilImg.show()
	pilImg.save(os.path.join(imgDir,"textextraction/ccmerged",exactImageLoc[:-4]+"-ccmerged.png"))
	
	mybnumbers=010.1.1.136.4226-Figure-5-mod.png
	for item in finalBoundingboxes:
		thisarea=(item[2]-item[0])*(item[3]-item[1])
		thisareamin=min((item[2]-item[0]),(item[3]-item[1]))
		mybnumbers+=(thisareamin-5)*thisarea
