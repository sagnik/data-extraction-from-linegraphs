...
rgb=io.imread('10.1.1.186.3729-Figure-5-mod.png')[:,:,0:3]
print rgb.shape
#show_img(rgb)
lab = color.rgb2lab(rgb)
print lab.shape
#colordict={}
#convert the lab array into a data set

allcolors=[]
for i in range(0,lab.shape[0]):
	for j in range(0,lab.shape[1]):
		allcolors.append(lab[i][j])

allcolors=np.asarray(allcolors).astype(int)
print allcolors.shape
kmeans = KMeans(init='k-means++', n_clusters=5, n_init=10)
labels=kmeans.fit_predict(allcolors)
print type(labels),labels.shape,np.max(labels),np.min(labels),labels[0],allcolors[0]
'''
