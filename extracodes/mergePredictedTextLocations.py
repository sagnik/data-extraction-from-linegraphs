...
def main():
	...
	+"goldstandardfortext/textpredictedhueboxlocs/words-threshold/"+actualimage[:-4]+"-textpredictedhue.box"
	imagetobesaved="/home/sagnik/linegraph-experiment-1/pngs/colorfigures/training/textextraction/"\
	+"goldstandardfortext/textpredictedhueimage/words-threshold/"+actualimage[:-4]+"-textpredictedhue.png"
	...
	HORIZONTALTHRESHOLD=np.min(horizontaldistances)+1
	VERTICALTHRESHOLD=np.min(verticaldistances)+1
	...
	for item in finalBoundingboxes:
		draw.rectangle([item[1]-1,item[0]-1,item[3]+1,item[2]+1],outline="red")
