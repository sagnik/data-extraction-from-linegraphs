...
def getDistHorizontal(Q,R):
	...
	if (Q[0]>R[2]):
		dX2=Q[0]-R[2]
	#if (dX1+dX2)<2: #vertical distance is very short between the rectangles, not checked in Saurabh's implementation
	return (dY1+dY2)
	#else:
	#	return 0
...
def getDistVertical(Q,R):
	...
	if (Q[0]>R[2]):
		dX2=Q[0]-R[2]
	#if (dY1+dY2)<2: #horizontal distance is very short between the rectangles, not checked in Saurabh's implementation
	return (dX1+dX2)
	#else:
	#	return 0
...
def caculateMergeProbability(item1,item2,horizontalmean,verticalmean,horizontalstd,verticalstd):
	vertdistance=getDistVertical(item1,item2)
	horizdistance=getDistHorizontal(item1,item2)
	horzcoef=-1*(np.square(horizdistance-horizontalmean)/(2*np.square(horizontalstd)))
	vertcoef=-1*(np.square(vertdistance-verticalmean)/(2*np.square(verticalstd)))
	val=np.exp(horzcoef)*np.exp(vertcoef)
	return val
	
def mergeOnce(imgray,finalBoundingboxesFiltered,horizontalmean,verticalmean,horizontalstd,verticalstd):
	...
	for i in range(0,len(finalBoundingboxesFiltered)):
		...
		for j in range(i+1,len(finalBoundingboxesFiltered)):
			...
			if caculateMergeProbability(item1,item2,horizontalmean,verticalmean,horizontalstd,verticalstd)>0.05:
				nomerges+=1
...
def getcandidateCCs(imgLoc):
	...
	for item in temp:
		if not (item[2]-item[0])*(item[3]-item[1]) > 0.2*imgarea:
...
def splitPlot(img,imgloc):
	horizontalprofile=[]
	verticalprofile=[]
	print img.shape,img[0][0]
	for i in range(0,img.shape[0]):
		horizontalprofile.append(np.sum(img[i,:]))
	for j in range(0,img.shape[1]):
		verticalprofile.append(np.sum(img[:,j]))
	m = max(horizontalprofile)
	horizontalsplitindex=sorted([i for i, j in enumerate(horizontalprofile) if j == m])[-1]
	#print "horizontal",m,horizontalsplitindex,horizontalprofile[:10],len(sorted([i for i, j in enumerate(horizontalprofile) if j == m]))
	m = max(verticalprofile)
	verticalsplitindex=sorted([i for i, j in enumerate(verticalprofile) if j == m])[0]
	#print "vertical",m,verticalsplitindex,verticalprofile[:10],len(verticalprofile)
	#xregbb=[0,horizontalsplitindex-2,img.shape[0],img.shape[1]]
	#yregbb=[0,0,img.shape[1],verticalsplitindex+2]
	#plotregbb=[verticalsplitindex-2,0,img.shape[1],horizontalsplitindex+2]
	im=Image.open(imgloc)
	#draw=ImageDraw.Draw(im)
	#draw.rectangle([0,horizontalsplitindex-2,im.size[0],im.size[1]],outline="green")
	#draw.rectangle([0,0,verticalsplitindex,im.size[1]],outline="blue")
	#draw.rectangle([verticalsplitindex+2,0,im.size[0],horizontalsplitindex],outline="red")
	#im.show()	
	xregbb=[horizontalsplitindex,0,im.size[1],im.size[0]]
	yregbb=[0,0,im.size[1],verticalsplitindex]
	plotregbb=[0,verticalsplitindex,horizontalsplitindex,im.size[0]]
	#show_img(img[xregbb[0]:xregbb[2],xregbb[1]:xregbb[3]])
	#show_img(img[yregbb[0]:yregbb[2],yregbb[1]:yregbb[3]])
	#show_img(img[plotregbb[0]:plotregbb[2],plotregbb[1]:plotregbb[3]])
	#print xregbb,yregbb,plotregbb
	return(xregbb,yregbb,plotregbb)
	
def getWords(imageloc,finalBoundingboxesFiltered):
	img=io.imread(imageloc)
	imgray=color.rgb2gray(img)
	binary=canny(imgray).astype(np.uint8)
	(xaxesregion,yaxesregion,plotregion)=splitPlot(binary,imageloc)
	horizontaldistances=[]
	verticaldistances=[]
	...
	for item in finalBoundingboxesFiltered:
		if boundingBoxiConsumedbyj(item,xaxesregion) or boundingBoxiConsumedbyj(item,yaxesregion): 	
			nottobemerged.append(item)
		else:
			tobemerged.append(item)
	print "going to merge",len(tobemerged),"CCs"	
	#print horizontaldistances,verticaldistances
	...
	nomerges=1
	#while(nomerges):
	horizontalmean=np.mean(horizontaldistances)
	verticalmean=np.mean(verticaldistances)
	horizontalstd=np.std(horizontaldistances)
	verticalstd=np.std(verticaldistances)
	(finalBoundingboxesFiltered,nomerges)=mergeOnce(imgray,tobemerged,horizontalmean,verticalmean,horizontalstd,verticalstd)	

	#finalBoundingboxes=finalBoundingboxesFiltered
	return finalBoundingboxesFiltered+nottobemerged	
	
def main():
	imgLoc=sys.argv[1]
	imgDir,exactImageLoc=os.path.split(imgLoc)
	basedir="/home/sagnik/linegraph-experiment-1/pngs/colorfigures/evaluation/textextraction/finalpredictedwords-doceng/saurabh/"
	...
	if candidateCCs:
		...
		filteredCCs=candidateCCs#filterCCs(imgLoc,candidateCCs)
	else:
